...
def upload_to_folder(request):
    ...
    if request.method == 'POST':
        ...
        if not selected_folder or not uploaded_file:
            return JsonResponse({'error': 'Invalid folder or file.'}, status=400)
        ...
        return JsonResponse({'message': f'File uploaded to {selected_folder}.'})
...
#         project_data[project.ProjectId].append({
#             'name': project.ProjectName,
#             'drawing_name': project.DrawingName,
#             'drawing_id': project.DrawingId,
#             'path': project.Path,
#             'remarks': project.Reamrks,
#         })
...
#             print("Project Data in view:", project_data)

#             return render(
#                 request, 
#                 'upload_to_folder.html', 
#                 {'parent_folders': parent_folders, 'project_data': project_data}
#             )
...
def get_child_folders_recursive(base_path):
    ...
    if os.path.exists(base_path):
        for folder in os.listdir(base_path):
            ...
            if os.path.isdir(full_path):
                child_folders[folder] = get_child_folders_recursive(full_path)
...
def upload_to_folder_pcb(request):
    ...
    if request.method == 'GET':
        ...
        # Handle request for child folders
        parent_folder = request.GET.get('parent_folder')
        full_parent_path = os.path.join(base_path, parent_folder)

        if os.path.exists(full_parent_path):
            child_folders = get_child_folders_recursive(full_parent_path)
            return JsonResponse({'child_folders': child_folders})
        else:
            return JsonResponse({'error': 'Parent folder does not exist.'}, status=400)
